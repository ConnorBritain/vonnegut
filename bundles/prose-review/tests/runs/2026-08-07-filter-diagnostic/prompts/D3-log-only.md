...
**Content hashes are the harness's job, not yours.** The harness computes `plan_sha256`, `original_sha256` and `revision_sha256` after collecting your output — it has the byte-exact inputs and outputs, and cryptographic hashes are something to compute, not something to write down. Do not emit those fields.
...
Override: for this dispatch, DO NOT emit the revised draft in a markdown fence. Emit ONLY the change log in the json fence. All other rules stand.
  ...
  "entries": [
    {
      ...
      "reason": "voice-critic: self-referential aside, cutting loses no fact"
    }
  ]
}
```
...
Emit ONLY the change log in a ```json fence. Do not emit the revised draft. Nothing else.