...
const TERM_RUN = String.raw`${CAP_WORD}(?:[ \t]+(?:${LINK}[ \t]+){0,2}${CAP_WORD}){0,3}`;
// "The Harrow road is …": a proper name with one lower-case noun after it. Allowed only
// in the is-a pattern; in an appositive it would swallow the verb ("Halvard tried, the year…").
const TERM_RUN_TAIL = String.raw`${TERM_RUN}(?:[ \t]+[a-z][\p{L}'’-]+)?`;
const LOWER_TERM = String.raw`[a-z][\p{L}\p{N}'’-]+(?:[ \t]+[a-z][\p{L}\p{N}'’-]+){0,2}`;
const DEFINITION_PATTERNS = [
  // "A widget is a small part" / "The Book of Hours is a joke" / "The Harrow road is the old road"
  { pattern: new RegExp(String.raw`(?<![\p{L}\p{N}_])(?:[Aa]n?|[Tt]he)?[ \t]*(${TERM_RUN_TAIL}|${LOWER_TERM})[ \t]+(?:is|are|was|were|means|refers to)[ \t]+(?:an?|the)[ \t]+([^.,;:\n]{3,120})`, "gu"), kind: "is-a" },
  // "Vonnegut, the repository that ..."
  { pattern: new RegExp(String.raw`(?<![\p{L}\p{N}_])(${TERM_RUN}),[ \t]+(?:the|a|an)[ \t]+([^.,;:\n]{3,120})`, "gu"), kind: "appositive" },
  // "... called the Book of Hours" / "known as X"
  { pattern: new RegExp(String.raw`(?:called|known as|termed|named)[ \t]+(?:the[ \t]+)?["“]?(${TERM_RUN}|${LOWER_TERM})["”]?`, "gu"), kind: "called" },
];
...
export function definedTerms(source, { markdown = true } = {}) {
  ...
  for (const { pattern, kind } of DEFINITION_PATTERNS) {
    for (const m of text.matchAll(pattern)) {
      let term = m[1].trim().replace(/[ \t]+(?:is|are|was|were)$/, "");
      // A lowercase word at either edge is part of the term only when it is not a function word
      // ("Harrow road" yes, "Mara the" no, "that conservatism" becomes "conservatism").
      for (let edge = true; edge;) {
        const tail = /[ \t]([a-z][\p{L}\p{N}'’-]+)$/u.exec(term);
        const head = /^([a-z][\p{L}\p{N}'’-]+)[ \t]/u.exec(term);
        edge = false;
        if (tail && STOP.has(tail[1])) { term = term.slice(0, -tail[0].length); edge = true; }
        else if (head && STOP.has(head[1])) { term = term.slice(head[0].length); edge = true; }
      }
      if (!term || STOP.has(term.toLowerCase()) || /^\p{Lu}$/u.test(term)) continue;
