#!/usr/bin/env node
/** Locked deterministic-conformance diagnostic for every model stage. Never acceptance evidence. */
...
function prepare() {
  ...
  write(schemas.conformance, assertStrictOutputSchema(DRAFT_SOURCE_SCHEMA, "conformance schema"));
...
async function dispatch() {
  ...
  const conformed = normalizeVoiceDraftSource(conformanceRecord.structured_output, {
    request: manifest.prompt,
  });
  if (!conformed.ok || conformed.refusal) {
    throw new Error(`conformance source failed: ${conformed.errors.join("; ") || "unexpected refusal"}`);
  }
  const finalConformance = measureDraftConformance(conformed.source.draft, targetCard);
