#!/usr/bin/env node
/** Locked one-cell diagnostic for every model stage. Never acceptance evidence. */
...
import {
  claimAuditPrompt, CODEX_NO_TOOLS_CONFIG, codexCompanionArtifactFields,
  committedManifestError, completedResult,
  criticPrompt, dispatchCodex, draftPrompt, invocationInput, lockedImplementationErrors,
...
function prepare() {
  ...
  if (design.harness !== "codex" || design.acceptance_evidence !== false || design.redraws !== 0
    ...
    throw new Error("DESIGN.json does not preserve the four-stage Codex diagnostic boundary");
  }
  ...
  write(schemas.profile, sourceRenderSchema(measurements));
  write(schemas.draft, DRAFT_SOURCE_SCHEMA);
  write(schemas.claim_audit, AUDIT_SCHEMA);
...
async function dispatch() {
  ...
  const assembledDraft = assembleVoiceDraft(normalized.source, { request: manifest.prompt });
  ...
  const auditPromptPath = stagePrompt(join(RUN, "prompts", "claim-audit.md"), claimAuditPrompt(
    { id: "stage-canary", prompt: manifest.prompt }, normalized.source,
  ));
  const auditRaw = join(RUN, "raw", "claim-audit.json");
  const auditRecord = await call(manifest, "claim_audit", auditPromptPath, auditRaw, {
    draft_raw_sha256: SHA(text(draftRaw)),
  });
  const auditApplied = applyVoiceDraftClaimAudit(normalized.source, auditRecord.structured_output, {
  ...
  const criticRecord = await call(manifest, "critic", criticPromptPath, criticRaw, {
    draft_raw_sha256: SHA(text(draftRaw)), audit_raw_sha256: SHA(text(auditRaw)),
  });
  ...
  const result = {
    ...
    profile_assembled: true, draft_assembled: true,
