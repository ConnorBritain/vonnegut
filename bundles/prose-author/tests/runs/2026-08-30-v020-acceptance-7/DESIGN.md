# prose-author v0.2.0 acceptance — locked design, complete rerun 7
...
- Every non-refusal candidate then receives exactly one mandatory semantic-revision call
  ...
  every semantic-bearing measured band. Requested length and its working interval remain explicit,
  measured operational targets, but are not promoted into a structural release gate: neither the
  pre-registered bar nor this run's locked bar contains a word-count threshold. Rerun 6 and two
  live canaries record why one revision cannot reliably enforce the self-added ±15% experiment.
...
Prompt length remains measured and reported as request-following evidence. It is deliberately
not a seventh ship criterion. The exact patch still cannot move a draft farther from its requested
word target, and the release notes must disclose material length deviations observed in this run.
