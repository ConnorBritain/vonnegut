...
const MARKER = "# Generated by vonnegut/install-prose-codex.mjs.";
const LEGACY_MARKER = "# Generated by agent-primitives/install-prose-codex.mjs.";
const MARKETPLACE = "vonnegut";
const PLUGINS = ["prose-author", "prose-tell-scan", "prose-review", "prose-outline", "prose-bible", "prose-research"];
const AGENTS = [
...
Options:
  --check        Verify the installed plugins and generated custom agents.
...
function installPlugins({ local = true, replaceMarketplace = false } = {}, run = runCodex) {
  ...
  const desired = local ? REPO : "ConnorBritain/vonnegut";
  const matches = prior && (local
    ? prior.marketplaceSource?.sourceType === "local" && realpathSync(prior.root) === realpathSync(REPO)
    : prior.marketplaceSource?.sourceType === "git" && prior.marketplaceSource.source === "https://github.com/ConnorBritain/vonnegut.git");
  if (prior && !matches) {
...
function installAgents() {
  ...
  for (const name of AGENTS) {
    ...
    if (existsSync(target)) {
      ...
      const unchangedLegacy = current === `${LEGACY_MARKER}\n${generatedWithoutMarker}`;
      if (!current.startsWith(`${MARKER}\n`) && current !== generatedWithoutMarker && !unchangedLegacy) {
...
export { AGENTS, PLUGINS, HOST_BOUNDARY, MARKER, parseArgs, readPrimitive, renderAgent, installPlugins, checkPlugins, verifyDeployment };
